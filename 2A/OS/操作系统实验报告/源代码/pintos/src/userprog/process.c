...
// 利用tid找到的线程, 没找到则为NULL
static struct thread *matching_thread;
// 在read_list里找到的线程的tid, 没找到为-1
...
process_execute (const char *file_name) 
{
  ...
  if (tid == TID_ERROR)
  {
    ...
    list_push_front(&thread_current()->child_process, &matching_thread->child_elem);
...
start_process (void *file_name_)
{
  ...
  *p-- = 0;
  ...
  if_.esp = esp;
  // 指针入栈
  // put a zero to prevent no parameter
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  // iterator of child thread list;
  struct list_elem *temp;

  // 没有子进程, 不等
  if(list_empty(&thread_current()->child_process)){
    return -1;
  }
  for(temp = list_front(&thread_current()->child_process); temp != NULL; temp = temp->next){
  ...
  return child_thread->return_value;
}
